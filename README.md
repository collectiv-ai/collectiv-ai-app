...
Focus: allow **separate spaces** for cities, NGOs, universities, projects.

Planned ideas:

- Spaces for different contexts:
  - e.g. city pilots, NGO boards, university councils, research projects
- Role per space:
  - a user can be a citizen in one space, an expert in another, an institution in a third
- Custom rules per space:
  - different voting durations, quorums or thresholds
...
Focus: make data usage transparent and user-controlled.

Planned ideas:

- Personal civic profile:
  - topic preferences, accessibility options, language settings
  - full export / delete options
- Voluntary data donation:
  - users can optionally donate anonymised interaction data
    for research or improving democratic processes
- Transparent data paths:
  - show which AI provider was used for which task
  - clarify which data stays local and which is sent to remote APIs

---

### 🔹 Level 7 – Lab & Simulation Mode

Focus: turn CollectiVAI into a **democracy lab** for experimentation.

Planned ideas:

- Experimental voting methods:
  - approval voting, ranked choice voting, liquid democracy, etc.
- What-if simulations:
  - explore consequences of different outcomes and parameter changes
- Clear separation from “real” governance:
  - lab mode is a safe sandbox
  - selected lab results can later be promoted to real on-chain proposals
